...
import edu.illinois.cs.cogcomp.cs546ccm2.evaluation.MatchCriteria.RelatednessMeasurer;

public class RelatednessMetrics {

	private static float[] getOutput(List<RelatednessRecord> output, RelatednessMeasurer rel) {
	...
	public static float getQuadraticDistance(List<RelatednessRecord> goldStandardList, RelatednessMeasurer rel) {
	...
	public static float getAbsoluteDistance(List<RelatednessRecord> goldStandardList, RelatednessMeasurer rel) {
		float[] output = getOutput(goldStandardList, rel);
		float[] goldStandard = getGoldStandard(goldStandardList);
		if (output.length != goldStandard.length) {
			throw new NullPointerException();
