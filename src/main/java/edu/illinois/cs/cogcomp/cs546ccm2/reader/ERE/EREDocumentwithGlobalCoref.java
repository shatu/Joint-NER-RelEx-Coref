...
public class EREDocumentwithGlobalCoref extends EREDocument implements Serializable {
...
//			String coreMention = target.getSurfaceString();
	...
	private void buildIndexFromAnnotationCoref () {
		 List<EREEntity> entityList = ereAnnotation.entityList;
		 for (int i = 0; i < entityList.size(); ++i) {
			 List<EREEntityMention> entityMentionList = entityList.get(i).entityMentionList;
			 
			 if (entityMentionList.size() > 0) {
				 int corefStart = entityMentionList.get(0).offset;
				 int corefEnd = entityMentionList.get(0).offset + entityMentionList.get(0).length;
				 for (int j = 0; j < entityMentionList.size(); ++j) {
					 EREEntityMention entityMention = entityMentionList.get(j);

					 goldCorefStartMap.put(entityMention.offset, new Pair<Integer, Integer>(corefStart, corefEnd));
					 goldCorefEndMap.put(entityMention.offset + entityMention.length, new Pair<Integer, Integer>(corefStart, corefEnd));
