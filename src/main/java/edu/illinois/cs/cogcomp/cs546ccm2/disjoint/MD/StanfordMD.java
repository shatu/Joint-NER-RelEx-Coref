...
public class StanfordMD implements AMentionDetector {
	...
	public void labelText(TextAnnotation ta) throws AnnotatorException {
		...
		for(Constituent cont : ta.getView(CCM2Constants.StanfordNER).getConstituents()) {
			view.addSpanLabel(cont.getSpan().getFirst(), cont.getSpan().getSecond(), "Stanford Mention", 1d);
		}
