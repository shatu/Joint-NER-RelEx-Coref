...
public class PairILFeatGen extends AbstractFeatureGenerator 
implements Serializable{

	private static final long serialVersionUID = -5902462551801564955L;
	public Lexiconer lm = null;
	
	public PairILFeatGen(Lexiconer lm) {
		this.lm = lm;
	}
	...
	public List<Pair<String, Double>> pairQuantityFeatures(PairX x) {
		...
		if(rate2 != null && Tools.getNumTokenMatches(Arrays.asList(unit1.split(" ")), 
			...
			features.add(new Pair<String, Double>("CommonTokenInRate2", 1.0));
		}
		if(rate1 != null && Tools.getNumTokenMatches(Arrays.asList(unit2.split(" ")), 
				Tools.getTokensList(rate1))>0) {
			features.add(new Pair<String, Double>("CommonTokenInRate1", 1.0));
		}
		if(Tools.getValue(x.quantities.get(x.quantIndex1)) > Tools.getValue(
